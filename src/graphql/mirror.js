...
export class Mirror {
   ...
   *           author: {
   *             id: "user:longtime-user-4",
   *             login: "longtime-user-4",
   *           },
   *         },
   *         {
  ...
  extract(rootId: Schema.ObjectId): mixed {
    ...
    return _inTransaction(db, () => {
      ...
      const dependenciesTablePrefix = "tmp_dependencies_";
      ...
      try {
        ...
        const neverUpdatedConnection: void | {|
          +id: Schema.ObjectId,
          +fieldname: Schema.Fieldname,
        |} = db
          .prepare(
            dedent`\
              SELECT object_id, fieldname
              FROM ${temporaryTableName}
              JOIN objects USING (id)
              JOIN connections ON objects.id = connections.object_id
              WHERE connections.last_update
            `
          )
          .get();
